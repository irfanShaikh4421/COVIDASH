...
const ChangeCountry = () => {
	...
	async function handleCountryChange(elem) {
		console.log(
			`Country code: ${location.countryCode} to be replaced with: ${elem.target.value}`
		);
	...
	async function handleStateChange(elem) {
		console.log(
			`State abbr.: ${location.state} to be replaced with: ${elem.target.value}`
		);
		try {
	...
	return (
		<p>
			...
			{location.countryCode === 840 ? (
				<select defaultValue={location.state} onChange={handleStateChange}>
					{stateDropDown}{' '}
				</select>
			) : null}
		</p>
